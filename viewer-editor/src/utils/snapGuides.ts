 ...
 * Includes horizontal, vertical, parallel, and orthogonal guides.
 */

import type { SnapGuide, GuideSnapResult, ViewportBounds } from '../types/snap';
 ...
 *
 * Creates:
 * - Horizontal guide through each vertex
 * - Vertical guide through each vertex
...
export function generateSnapGuides(
  ...
  // Add guides through each additional vertex
  if (coordinates.length >= 2) {
    for (let i = 1; i < coordinates.length; i++) {
      const point = coordinates[i];

      // Horizontal guide through this vertex
      guides.push({
        id: generateGuideId('horizontal', i),
        origin: point,
        direction: [1, 0],
        type: 'horizontal'
      });

      // Vertical guide through this vertex
      guides.push({
        id: generateGuideId('vertical', i),
        origin: point,
        direction: [0, 1],
        type: 'vertical'
      });
...
/**
 * Find all intersection points between guides (deduplicated)
 */
export function findGuideIntersections(guides: SnapGuide[]): [number, number][] {
  ...
  for (let i = 0; i < guides.length; i++) {
    for (let j = i + 1; j < guides.length; j++) {
      ...
      if (intersection) {
        // Avoid duplicates (round to 6 decimal places for comparison)
        const key = `${intersection[0].toFixed(6)},${intersection[1].toFixed(6)}`;
        if (!seen.has(key)) {
          seen.add(key);
